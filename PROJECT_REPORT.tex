...
\author{Nuhin (Computing Languages Final Project)}
\date{\today}
...
Q-learning is a reinforcement learning algorithm where an agent learns what to do by trying actions and receiving rewards. In my project, each round of negotiation is a ``step.'' The buyer is the agent, and the seller (me) is the environment. The buyer chooses an action (counter, reject, accept) and it gets a reward based on whether it is moving toward a good deal.
...
\textbf{Important improvement I made after testing:} I noticed the TF-IDF buyer could bring up the wrong item (e.g., Blu-ray/stereo talk during a soda negotiation). To fix that, I added an \textbf{item grounding filter}: the query is augmented with the item name and candidate utterances that mention a different likely item are filtered out.
...
\textbf{Solution}: I added \textbf{item grounding} to the TF-IDF generator. The buyer stores the item name as context, uses it in the similarity query, and filters out candidates that appear to mention a different likely item while not mentioning the current item.
